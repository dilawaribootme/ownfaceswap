...
# -------------------------
# NEW: download URL images and inject into top-level images list
# -------------------------
def download_and_prepare_images_for_workflow(workflow, input_images):
    """
    Scans the workflow for nodes that contain a 'url' input and:
      - downloads the image,
      - appends it to input_images (list of dicts with 'name' and 'image'),
      - replaces the node with a LoadImage node that references ["INPUT_IMAGE", index]
    """
    ...
    for node_id, node_data in list(workflow.items()):
        try:
            ...
            # If a node carries a 'url' field, we'll treat it as an external image node
            if "url" in inputs and inputs.get("url"):
                url = inputs.get("url")
                print(f"worker-comfyui - Found URL at node {node_id}: {url}")
                try:
                    ...
                    img_obj = {"name": f"node_{node_id}.png", "image": f"data:image/png;base64,{encoded}"}
                    # index where this image will be available to ComfyUI after upload
                    img_index = len(input_images)
                    input_images.append(img_obj)
                    # replace this node with a LoadImage node that points to top-level input image
                    workflow[node_id] = {
                        "inputs": {"image": ["INPUT_IMAGE", img_index]},
                        "class_type": "LoadImage",
                        "_meta": {"title": "Load Image"}
                    }
                    print(f"worker-comfyui - Node {node_id} replaced with LoadImage -> INPUT_IMAGE index {img_index}")
                except Exception as e:
                    print(f"worker-comfyui - Failed to download/convert image from {url}: {e}")
                    # leave the node intact; ComfyUI will error later (we log and continue)
        except Exception as e:
            print(f"worker-comfyui - Error while scanning node {node_id}: {e}")
...
def handler(job):
    ...
    input_images = validated_data.get("images") or []

    # Convert any URL nodes to uploaded images + LoadImage nodes
    workflow, input_images = download_and_prepare_images_for_workflow(workflow, input_images)

    # Ensure ComfyUI HTTP API is reachable
    ...
    # Upload input images if present
    if input_images:
